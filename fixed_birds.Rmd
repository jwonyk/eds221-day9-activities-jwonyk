...
The code below reads in and assigns multiple CSV files: (here::here)
```{r, include = FALSE}

# Create a list of file names you want to read in (o)
...
Basic exploration of the bird observation dataset. There is only one error in the code chunk below (i.e., not all individual lines have an error). (unique(birds_1 & species_id))
```{r, include = FALSE}
...
# See the different species (species_id) (o)
...
The code below should join the information in `taxalist_1` to the `birds_1` data frame, by the `species_id` key. (by = "species")
...
The code below joins the site information to `birds_df`, then groups by and `park_name`, then finds the sum of the `bird_count` column. The top 15 values are retained, and the park_name converted to an ordered factor by total count. (group_by(park_name))
...
This code creates a plot of the top 15 parks by total bird count. (y = "Park name") %>% -> +)
```{r}
...
The code below joins the site information to the bird count data, then finds the top five most observed birds (by frequency) at each park. (full join?)

```{r}
common_birds <- birds_df %>% 
  full_join(sites_1, by = "site_id") %>%  
  group_by(common_name, park_name) %>%
  summarize(total_count = sum(bird_count, na.rm = TRUE)) %>%
...
The code below creates a faceted ggplot graph with the top 5 most frequently observed birds at each location. (aes())

```{r}
ggplot(data = selected_parks, aes(x = total_count, y = common_name)) +
  geom_col() +
...
The code below exports the graph as a .jpg and saves in the project root. (here::, heigth)
```{r}
